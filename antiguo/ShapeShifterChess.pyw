...
def mostrar_menu_opciones(piezas, piezas_activas, config):
    ...
    gamelib.draw_rectangle(2, 2, config['ancho'] + 1, alto + 1, outline = '#ff1955', width = '2', fill = '#181818', stipple = 'gray12')
...
def main():
    ...
    while gamelib.is_alive():
        ...
        if not ev: #el usuario cierra la ventana
            break

        if ev.mouse_button != 1: # se presiona un boton del mouse que no es el click izquierdo
            continue

        if menu == 0: #dentro del menu principal
