...
def mostrar_menu_opciones(piezas, piezas_activas, config):
    ...
    gamelib.draw_image(f'img/piezas{alto}.gif', unidad * 2, unidad * 5)
    ...
    for n, pieza in enumerate(piezas):
        gamelib.draw_image (f'img/{pieza}_blanco{alto}.gif', unidad * (2 + n % 3) + unidad // 2, unidad * (6 + n // 3))
        if pieza in piezas_activas: 
            #2 filas de 3 columnas
            gamelib.draw_rectangle(unidad * (2 + n % 3) + unidad // 2, unidad * (6 + n // 3), unidad * (3 + n % 3) + unidad // 2, unidad * (7 + n // 3), outline = '#ff1955', width = '2', fill = '')
        else:
            gamelib.draw_rectangle(unidad * (2 + n % 3) + unidad // 2, unidad * (6 + n // 3), unidad * (3 + n % 3) + unidad // 2, unidad * (7 + n // 3), outline = '#ff1955', width = '2', fill = 'black', stipple = 'gray75')
...
def main():
    ...
    evento_r = gamelib.wait(gamelib.EventType.Leave)
    canva = evento_r.widget
    prueba_box = [0]
    prueba = lambda a: (prueba_box.append(prueba_box[0]+1),prueba_box.pop(0), print(prueba_box[0]))
    canva.event_add('<<panic>>', '<Button-3>', '<KeyPress-r>')
    canva.bind('<<panic>>', prueba)
 
    k = gamelib.wait(gamelib.EventType.MouseWheel)
    canva['highlightthickness']=0
    print('pase')
    ...
    while gamelib.loop(21):
        ...
        for evento in gamelib.get_events():
            if evento.type == gamelib.EventType.ButtonPress:
                ev = evento
            if evento.type == gamelib.EventType.MouseWheel:
                print('a')
            if evento.type == gamelib.EventType.Leave:
                z+=1
                evento.type = gamelib.EventType.MouseWheel
            if evento.type == gamelib.EventType.Visibility:
                print('c')
            if evento.type == 'panic':
                print('d')
        ...
        if menu == 2 and ev:
            ...
            elif unidad * 2 + unidad // 2 < ev.x < unidad * 6 - unidad // 2 and unidad * 6 < ev.y < unidad * 8:
                #se clickea sobre la caja de piezas
                posicion = ev.x // unidad - 2 - (ev.x // (unidad // 2) - 1) % 2, ev.y // unidad -  6
